...
def test_state_file_was_not_modified_by_this_change():
    """The spec/changes both assert state/blog.json was untouched. Confirm via
    git that it is not part of the working-tree diff."""
    result = subprocess.run(
        ["git", "diff", "--name-only", "HEAD", "--", "state/blog.json"],
        cwd=str(REPO_ROOT),
        capture_output=True,
        text=True,
    )
    assert result.returncode == 0
    assert result.stdout.strip() == "", "state/blog.json was modified, but the spec says it must not be."
...
def test_only_config_yaml_modified_in_working_tree():
    """git status should show no unexpected modified/added/deleted files.
    Only config.yaml (the new topic) and the test CI wiring are in scope;
    anything else is out of scope per the spec."""
    result = subprocess.run(
    ...
    lines = [
        ...
        # Ignore this tests/ directory, .pipeline/ artifacts, and the test
        # CI workflow — those are deliberate additions alongside the coder's
        # config.yaml change, not out-of-scope drift.
        if not line.strip().endswith("tests/")
        and "tests/test_blog_topic_config.py" not in line
        and ".pipeline/" not in line
        and "framer/node_modules" not in line
        and "framer/package-lock.json" not in line
        and "tests/" not in line
        and ".github/workflows/tests.yml" not in line
    ]
    assert lines == [f" M config.yaml"], f"Unexpected working-tree changes: {lines}"


def test_published_dir_untouched():
    result = subprocess.run(
    ...
    assert result.returncode == 0
    assert result.stdout.strip() == "", "Something under published/ was modified/added, which is out of scope."


def test_no_python_or_mjs_files_modified():
    result = subprocess.run(
    ...
    bad = [f for f in changed if f.endswith(".py") or f.endswith(".mjs")]
    assert bad == [], f"Code files were modified, which is out of scope: {bad}"


def test_config_yaml_is_the_only_tracked_diff():
    result = subprocess.run(
        ["git", "diff", "--name-only", "HEAD"],
        cwd=str(REPO_ROOT),
        capture_output=True,
    ...
    assert result.returncode == 0
    changed = [f for f in result.stdout.splitlines() if f.strip()]
    assert changed == ["config.yaml"], f"Expected only config.yaml in tracked diff, got: {changed}"
